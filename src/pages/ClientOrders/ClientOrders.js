...
export const ClientOrders = () => {
  ...
  useEffect(() => {
    if (Array.isArray(orderReducer.buyerOrders)) {
      ...
      <Box height="100vh">
        ...
        <Center>
          <Tabs width="90%">
            ...
            <TabPanels>
              {Array.isArray(orderReducer.buyerOrders) && (
                <TabPanel id="Orders">
                  {orderReducer.buyerOrders.length === 0 && (
              ...
              {Array.isArray(orderReducer.buyerOrders) && (
                <TabPanel id="Pending-Orders">
                  {orderReducer.buyerOrders.length === 0 && (
                    <Text> There are no pending orders</Text>
                  )}
                  {pending}
                </TabPanel>
              )}
              {Array.isArray(orderReducer.cancelledOrders) && (
                <TabPanel id="Cancelled-Orders">
                  {orderReducer.cancelledOrders.length === 0 && (
                    <Text> There are no cancelled orders</Text>
                  )}
